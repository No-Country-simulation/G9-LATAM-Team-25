    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        const backendUrl =
          ...
          "https://g9-latam-team-25.onrender.com";
        ...
        try {
          ...
          if (!(file instanceof File)) {
            return new Response(
              JSON.stringify({
                error:
                  "El archivo no llegó correctamente al proxy del frontend.",
              }),
              {
                status: 400,
                headers: {
          ...
          console.error(
            "Error en proxy contenido-archivo:",
            err,
          );
